...
\chapter{\label{ch:2-cips}\chcips}
...
\end{equation}
as if a molecule is larger, then there are fewer \todo{JAC Q} molecules per volume and therefore fewer microstates. Volume effects will also appear in the calculation of the chemical potential, due to the conversion between volume fraction and the number of molecules
\begin{equation}
...
The final contribution considered here describes an energy of interfaces and is related to the surface tensions between domains of different compositions. Similarly to the interaction term, in the case of an $N$ component matrix, the interface contribution to the free energy will be composed \todo{JAC Q} of couplings between the gradient of the volume fractions of each component to give
\begin{equation}
...
\subsubsection{Enzymatic Activity}\label{sec:2-enz-act}
...
\end{pmatrix}
\end{equation}}}
where the dot indicates the time derivative. In this formulation, we can explicitly see the incompressibility being imposed as the columns of $\cal C$ sum to zero.
